            ...
            <div id="international" class="form-box shadow p-3" style="width: 95%; height: auto; background-color: white; border-radius: 5px; padding-bottom: 30px;">
                ...
                @foreach($news as $news)
                    <table class="my-3">
                        <thead>
                            <tr>
                                <th  colspan="4" style="text-align:left;">News Title: {{ $news->newstitle }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td colspan="4" style="text-align:left;">News Date: {{ $news->newsdate }}</td>
                            </tr>
                            <tr>
                                <td colspan="4" style="text-align:left;">News: {{ $news->news }}</td>
                            </tr>
                            <tr>
                                <td colspan="4">
                                    <div  class="d-flex justify-content-between">
                                        <div class="d-flex justify-content-center">
                                                <abbr title="See image">
                                                    <a href="{{ $news->newsimage}}" target="blank">
                                            ...
                                            <div class="mx-1"></div>
                                                <abbr title="Read article">
                                                    <a href="{{ $news->newsurl }}" target="blank">
                                        ...
                                        <div class="flex justify-content-center">
                                            ...
                                            @if($news->status == 'unpublish')
                                                <form method="POST" action="{{ route('newss.publish', $news->id) }}">
                                                    @csrf
                                                    <button type="submit" class="rounded bg-warning px-2 py-1 pb-[5px] pt-[6px] text-white">Publish</button>
                                                </form>
                                            @else
                                                <form method="POST" action="{{ route('newss.unpublish', $news->id) }}">
                                                    @csrf
                                                    <button type="submit" class="rounded bg-danger px-2 py-1 pb-[5px] pt-[6px] text-white">Unpublish</button>
                                                </form>
                                            @endif

                                            <a href="{{ route('newss.edit', $news->id) }}" class="rounded bg-primary px-2 py-1 pb-[5px] pt-[6px] text-white mx-1">Edit</a>
                                            <a href="{{ route('newss.edit', $news->id) }}" class="rounded bg-success px-2 py-1 pb-[5px] pt-[6px] text-white">View</a>
