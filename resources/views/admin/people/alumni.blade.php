...
<body>
   ...
   <div style="background-color: rgb(245, 245, 245); display: flex; flex-direction: column; justify-content: center; align-items: center; height: auto; padding: 30px 0 30px 0; margin-top: 60px;">
        <div id="international" class="form-box shadow p-3" style="width: 95%; height: auto; background-color: white; border-radius: 5px; padding-bottom: 30px;">
            <h5>Make changes on System Alumni</h5>
            ...
            <table class="my-3">
                ...
                <tbody>
                    ...
                    @foreach($alumni as $almn)
                    <tr>
                        <td>{{ $almn->id }}</td>
                        <td>{{ $almn->title }} {{ $almn->firstname }} {{ $almn->lastname }}</td>
                        <td>{{ $almn->email }}</td>
                        <td>{{ $almn->degree }} {{ $almn->studyarea }}</td>
                        <td>{{ $almn->startedyear }}</td>
                        <td>{{ $almn->endedyear }}</td>
                        <td>{{ $almn->rate }}</td>
                        <td>
                            <div class="d-flex justify-content-center">
                                <a href="{{ $almn->alumniimage}}" target="blank">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-image-fill" viewBox="0 0 16 16">
                                        <path d="M.002 3a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2h-12a2 2 0 0 1-2-2zm1 9v1a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V9.5l-3.777-1.947a.5.5 0 0 0-.577.093l-3.71 3.71-2.66-1.772a.5.5 0 0 0-.63.062zm5-6.5a1.5 1.5 0 1 0-3 0 1.5 1.5 0 0 0 3 0"/>
                                    </svg>
                                </a>
                                <div class="mx-1"></div>
                                <a href="{{ $almn->profileurl }}" target="blank">
                        ...
                        <td>
                            <div class="flex justify-content-center">
                                ...
                                @if($almn->status == 'unpublish')
                                    <form method="POST" action="{{ route('alumni.publish', $almn->id) }}">
                                        @csrf
                                        <button type="submit" class="rounded bg-warning px-2 py-1 pb-[5px] pt-[6px] text-white">Publish</button>
                                    </form>
                                @else
                                    <form method="POST" action="{{ route('alumni.unpublish', $almn->id) }}">
                                        @csrf
                                        <button type="submit" class="rounded bg-danger px-2 py-1 pb-[5px] pt-[6px] text-white">Unpublish</button>
                                    </form>
                                @endif

                                <a href="{{ route('alumni.edit', $almn->id) }}" class="rounded bg-primary px-2 py-1 pb-[5px] pt-[6px] text-white mx-1">Edit</a>
                                <a href="{{ route('alumni.edit', $almn->id) }}" class="rounded bg-success px-2 py-1 pb-[5px] pt-[6px] text-white">View</a>
                                <form action="{{ route('alumni.destroy', $almn->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
