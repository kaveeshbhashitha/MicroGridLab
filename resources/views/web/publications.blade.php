...
<head>
    ...
    <x-meta-tags-component
        title="Publication - Smart Grid Research Group"
...
<body>
                    ...
                    <div class="navbar-nav">
                        ...
                        <div class="nav-item dropdown">
                            ...
                            <div class="dropdown-menu bg-light m-0">
                                ...
                                @if(count($publication)>0)
                                    <a href="{{ route('publications') }}" class="dropdown-item active">Publication</a>
                                @endif
    ...
    @php
        $latestPublication = app(\App\Http\Controllers\PublicationController::class)->showPublicationhData();
    @endphp

    @if($latestPublication && count($latestPublication) > 0)
        @for($i = 0; $i < count($latestPublication); $i+=2)
            <div class="container-fluid container-team py-5">
                <div class="container pb-2">
                    <div class="row g-5 align-items-center mb-5">
                        <div class="col-md-6 wow fadeIn" data-wow-delay="0.3s">
                            <img class="img-fluid w-100" src="{{ $latestPublication[$i]->pubimage }}" alt="">
                        </div>
                        <div class="col-md-6 wow fadeIn" data-wow-delay="0.5s">
                            ...
                            <p class="mb-6 short-description" style="text-align:justify;">{{ substr($latestPublication[$i]->description, 0, 150) }}{{ strlen($latestPublication[$i]->description) > 150 ? '...' : '' }}</p>
                                ...
                                @endif
                            <div class="full-description d-none my-1" style="text-align:justify;">{{ $latestPublication[$i]->description }}</div>
                        
                            <div class="d-flex justify-content-end">
                                <abbr title="See article"><a class="btn btn-lg-square btn-primary me-2" href="{{ $latestPublication[$i]->puburl }}"><i class="fas fa-link"></i></a></abbr>
                            </div>
                        </div>
            ...
            <!-- Team Start -->
            @if(isset($latestPublication[$i+1]))
            <div class="container-fluid container-team py-5">
                <div class="container pb-2">
                    <div class="row g-5 align-items-center mb-5">
                        <div class="col-md-6 wow fadeIn" data-wow-delay="0.5s">
                            
                            <h3 class="mb-3">{{ $latestPublication[$i+1]->pubtitle }}</h3>
                            <h5 class="mb-3">{{ $latestPublication[$i+1]->pubname }}</h5>
                            <p class="mb-1">Published on: <a href="{{ $latestPublication[$i+1]->puburl }}">{{ $latestPublication[$i+1]->pubjournal }}</a></p>
                            <p class="mb-2">{{ $latestPublication[$i+1]->pubdate }}, Issue: {{ $latestPublication[$i+1]->issue }}, Volume: {{ $latestPublication[$i+1]->volume }}, Pages: {{ $latestPublication[$i+1]->pages }}</p>
                            
                            <p class="mb-6 short-description" style="text-align:justify;">{{ substr($latestPublication[$i+1]->description, 0, 150) }}{{ strlen($latestPublication[$i+1]->description) > 150 ? '...' : '' }}</p>
                                @if(strlen($latestPublication[$i+1]->description) > 150)
                                    <button class="rounded btn btn-sm btn-primary toggle-btn">Read More</button>
                                @endif
                            <div class="full-description d-none my-1" style="text-align:justify;">{{ $latestPublication[$i+1]->description }}</div>
                            
                            
                            <div class="d-flex justify-content-end">
                                <abbr title="See article"><a class="btn btn-lg-square btn-primary me-2" href="{{ $latestPublication[$i+1]->puburl }}"><i class="fas fa-link"></i></a></abbr>
                            </div>
                            
                        </div>

                        <div class="col-md-6 wow fadeIn" data-wow-delay="0.3s">
                            <img class="img-fluid w-100" src="{{ $latestPublication[$i+1]->pubimage }}" alt="">
                        </div>
                    </div>
                </div>
            </div>
    ...
    @elseif($latestPublication && count($latestPublication) > 0 && count($latestPublication) < 2 )

    @for($i = 0; $i < count($latestPublication); $i++)
            <div class="container-fluid container-team py-5">
                <div class="container pb-2">
                    <div class="row g-5 align-items-center mb-5">
                        <div class="col-md-6 wow fadeIn" data-wow-delay="0.3s">
